...
from app.pipeline.ingest_dpe import ingest_dpe_paris
...
async def run_full_pipeline(departement: str = "75", annee: int = 2024) -> dict:
    ...
    dpe_stats = await ingest_dpe_paris(annee=annee)

    log.info("pipeline.complete", raw=raw_stats, clean=clean_stats, dpe=dpe_stats)
    return {"raw": raw_stats, "clean": clean_stats, "dpe": dpe_stats}
